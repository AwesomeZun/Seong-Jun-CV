...
const exportName = document.querySelector('.cv-download')?.getAttribute('href')?.replace(/\.pdf$/, '') || 'Seong-Jun-Kang-CV';
      ...
      const expertiseHeading = doc.querySelector('.interest')?.closest('.sec')?.querySelector('.sechdr h2');
      if (expertiseHeading) expertiseHeading.textContent = 'Research & Technical Expertise';
      if (!location.pathname.includes('/for-')) {
        const profile = doc.querySelector('.profile');
        if (profile) {
          const label = profile.querySelector('.slab')?.cloneNode(true);
          profile.textContent = 'I am a computational immunologist integrating experimental immunology with single-cell, spatial, and multimodal data analysis. I have run GeoMx DSP, high-parameter flow cytometry, Biacore SPR, and 10x workflows, then analyzed the resulting data in Python and R with donor-level or held-out evaluation. At PB Immune Therapeutics, I established in-house single-cell, spatial, and proteomic workflows from scratch and led the anti-CD40 candidate evaluation program through non-human-primate studies.';
          if (label) profile.prepend(label);
        }
      }
      if (generalExpertise) {
        generalExpertise.innerHTML = '<b>Cellular and molecular assays.</b> Primary-cell culture and immune-cell functional assays; DNA, RNA, and protein workflows, including qRT-PCR, cloning, and Western blotting; Biacore SPR and ELISA binding assays.';
      }
      const multiModalExpertise = doc.querySelectorAll('.interest li')[0];
      if (multiModalExpertise) {
        multiModalExpertise.innerHTML = '<b>Single-cell and multimodal profiling.</b> 10x single-cell workflows, end-to-end GeoMx DSP, Visium spatial transcriptomics, MACSima multiplexed imaging, high-parameter flow cytometry, and Olink targeted proteomics.';
      }
      const computationalExpertise = doc.querySelectorAll('.interest li')[1];
      if (computationalExpertise) {
        computationalExpertise.innerHTML = '<b>Computational analysis and AI-driven drug discovery.</b> Sequencing-data analysis in Python and R using statistical and machine-learning tools; multimodal integration and donor- or case-level evaluation. AI-driven drug discovery workflows spanning target discovery, binding prediction, virtual screening, and lead optimization.';
      }
      const programExpertise = doc.querySelectorAll('.interest li')[3];
      if (programExpertise) {
        const animalExpertise = doc.createElement('li');
        animalExpertise.innerHTML = '<b>Animal models and immune monitoring.</b> EAE and Alzheimer\'s disease models; skin- and islet-transplantation studies; animal breeding, non-human-primate sampling, and longitudinal immune monitoring.';
        programExpertise.before(animalExpertise);
        programExpertise.innerHTML = '<b>Research coordination and grant writing.</b> Multi-site sample QC, project coordination across academia and industry, and contributions to grant proposals for programs with combined budgets of approximately $3 million.';
      }
