...
const exportName = document.querySelector('.cv-download')?.getAttribute('href')?.replace(/\.pdf$/, '') || 'Seong-Jun-Kang-CV';
      ...
      if (!location.pathname.includes('/for-')) {
        ...
        if (profile) {
          ...
          profile.textContent = 'I am a computational immunologist combining primary-cell experiments with single-cell, spatial, and multimodal data analysis. I have run GeoMx DSP, high-parameter flow cytometry, Biacore SPR, and 10x workflows, then analyzed the resulting data in Python and R with donor-level or held-out evaluation. At PB Immune Therapeutics, I established in-house single-cell, spatial, and proteomic workflows from scratch and led the anti-CD40 candidate evaluation program through non-human-primate studies.';
          if (label) profile.prepend(label);
        }
      }
      if (generalExpertise) {
        generalExpertise.innerHTML = '<b>Wet-lab and translational experiments.</b> Primary-cell culture and immune-cell functional assays; DNA, RNA, and protein workflows, including qRT-PCR, cloning, and Western blotting; SPR and ELISA binding assays.';
      }
      const multiModalExpertise = doc.querySelectorAll('.interest li')[0];
      if (multiModalExpertise) {
        multiModalExpertise.innerHTML = '<b>Multi-modal cellular measurement.</b> 10x single-cell workflows, end-to-end GeoMx DSP, Visium spatial transcriptomics, MACSima cyclic multiplexed immunofluorescence, high-parameter flow cytometry, and Olink targeted proteomics.';
      }
      ...
      if (programExpertise) {
        ...
        animalExpertise.innerHTML = '<b>Animal experiments.</b> EAE and Alzheimer\'s disease models; skin- and islet-transplantation studies; animal breeding, longitudinal immune monitoring, and non-human-primate sampling.';
        programExpertise.before(animalExpertise);
        programExpertise.innerHTML = '<b>Program and study execution.</b> Multi-site sample QC, proposal writing, and project execution supporting approximately $1.3 million in industry research funding.';
      }
